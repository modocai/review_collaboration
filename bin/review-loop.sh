...
EOF
  ...
  if [[ "$AUTO_COMMIT" == true ]]; then
    ...
    if [[ ! -s "$FIX_FILES_NUL_FILE" ]]; then
      ...
      # Collect pre-dirty paths into temp files for Bash 3-compatible lookup.
      _pre_dirty_lookup=$(mktemp)
      _pre_dirty_pathlist=$(mktemp)
      if [[ -s "$_pre_dirty_fix_file" ]]; then
        while IFS=$'\t' read -r -d '' _blob _path; do
          [[ -n "$_blob" ]] || continue
          printf '%s\t%s\0' "$_blob" "$_path" >> "$_pre_dirty_lookup"
          printf '%s\n' "$_path" >> "$_pre_dirty_pathlist"
        done < "$_pre_dirty_fix_file"

        while IFS=$'\t' read -r -d '' _pre_blob _path; do
          [[ -n "$_pre_blob" ]] || continue
          ...
          if [[ -n "$_head_blob" ]]; then
            ...
            if ! git merge-file -q "$_tmp_ours" "$_tmp_base" "$_tmp_theirs" 2>/dev/null; then
              echo "  ERROR: merge conflict in pre-dirty file: $_path" >&2
              rm -f "$_tmp_base" "$_tmp_ours" "$_tmp_theirs" "$_pre_dirty_lookup" "$_pre_dirty_pathlist" "$_pre_dirty_fix_file"
              echo "  Aborting commit due to merge conflict."
              continue 2  # skip to next iteration of the outer review loop
            fi
            # Stage the cleanly merged result
        ...
        done < "$_pre_dirty_lookup"
      fi
      rm -f "$_pre_dirty_fix_file"
      # Stage remaining (non-pre-dirty) files normally
      while IFS= read -r -d '' _f; do
        [[ -n "$_f" ]] || continue
        if ! grep -qxF "$_f" "$_pre_dirty_pathlist" 2>/dev/null; then
          git add -- "$_f"
        fi
      done < "$FIX_FILES_NUL_FILE"
      rm -f "$_pre_dirty_lookup" "$_pre_dirty_pathlist"
