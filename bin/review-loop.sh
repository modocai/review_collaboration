...
for (( i=1; i<=MAX_LOOP; i++ )); do
  ...
  codex exec \
    -a "read-only" \
    "$REVIEW_PROMPT" \
    > "$REVIEW_FILE" 2>&1 || true

  # ── c. Extract JSON from response ────────────────────────────────
  # The response may contain markdown fences or extra text; extract first JSON object
  REVIEW_JSON=$(sed -n '/^{/,/^}/p' "$REVIEW_FILE" | head -1000)

  # Fallback: try jq directly on the whole file
  if ! echo "$REVIEW_JSON" | jq empty 2>/dev/null; then
    REVIEW_JSON=$(jq -r '.' "$REVIEW_FILE" 2>/dev/null || true)
  fi
