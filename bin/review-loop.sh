...
EOF
  ...
  FIX_FILES_NUL=$( { git diff --name-only -z; git ls-files --others --exclude-standard -z; } | sort -zu )
  if [[ -z "$FIX_FILES_NUL" ]]; then
    echo "  No file changes after fix — nothing to commit."
  else
    echo "[$(date +%H:%M:%S)] Committing fixes..."
    printf '%s' "$FIX_FILES_NUL" | xargs -0 git add --
    printf '%s' "$FIX_FILES_NUL" | git commit --pathspec-from-file=- --pathspec-file-nul \
